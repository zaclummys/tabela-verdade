...
export default class ExpressionPresenterFactory {
    ...
    createFromBinaryExpression(expression) {
        ...
        if (expression instanceof AndExpression) {
            return new AndExpressionPresenter(left, right);
        }

        if (expression instanceof OrExpression) {
            return new OrExpressionPresenter(left, right);
        }

        if (expression instanceof ConditionalExpression) {
            return new ConditionalExpressionPresenter(left, right);
        }

        if (expression instanceof BiconditionalExpression) {
            return new BiconditionalExpressionPresenter(left, right);
        }
